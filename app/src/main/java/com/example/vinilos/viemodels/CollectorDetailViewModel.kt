...
class CollectorDetailViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val _collectorAlbumNames = MutableLiveData<List<String>>()
    val collectorAlbumNames: LiveData<List<String>>
        get() = _collectorAlbumNames
    ...
    fun loadCollectorAlbumNames(collectorId: Int) {
        Log.d("CollectorDetailViewModel", "Cargando álbumes del coleccionista con ID: $collectorId")
        _isLoading.value = true

        NetworkServiceAdapter.getInstance(getApplication())
            .getCollectorAlbumNames(
                collectorId,
                { albumNames ->
                    Log.d("CollectorDetailViewModel", "Álbumes cargados exitosamente")
                    _collectorAlbumNames.value = albumNames
                    _isLoading.value = false
                },
                { error ->
                    val errorMessage = when {
                        ...
                        error.message != null -> {
                            ...
                            Log.e("CollectorDetailViewModel", "Error desconocido: ${error.toString()}")
                            "Error desconocido al cargar los álbumes del coleccionista: ${error.toString()}"
                        }
                    }
                    _error.value = errorMessage
                    _isLoading.value = false
